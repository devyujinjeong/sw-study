package com.dbwls.springsecurity.auth.service;

import com.dbwls.springsecurity.auth.dto.LoginRequest;
import com.dbwls.springsecurity.auth.dto.TokenResponse;
import com.dbwls.springsecurity.auth.entity.RefreshToken;
import com.dbwls.springsecurity.jwt.JwtTokenProvider;
import com.dbwls.springsecurity.auth.repository.RefreshTokenRepository;
...
public class AuthService {
    ...
    public TokenResponse login(LoginRequest request) {
        ...
        // refreshToken은 서버측에서 관리 되어야 하는 데이터이고 성능 상 추천되는 환경은 Redis
        // RDBMS에 저장해서 관리하는 코드로 작성
        RefreshToken tokenEntity = RefreshToken.builder()
                .username(user.getUsername())
                .token(refreshToken)
                .expiryDate(new Date(System.currentTimeMillis()  // 필수는 아니지만, 이중 확인이고 보통 이럼
        ...
        return TokenResponse.builder()
                .accessToken(accessToken) // 서버에서 상태 관리 안함
                .refreshToken(refreshToken) // 서버에서 상태 관리함
                .build();
    }

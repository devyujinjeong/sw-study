...
// 클릭시 다크 모드가 true가 되도록 하는 함수
const toggleDarkMode = () => darkState.value = !darkState.value;

</script>

<template>
  <!-- 현재 darkState가 false이기 때문에 class명이 붙지 않는다. -->
  <div :class="{ container : true, dark : darkState }">
    ...
    <!-- DarkMode 컴포넌트 toggle 이벤트를 emit 하면 toggleDarkMode 함수가 실행된다. -->
    <DarkMode toggle="toggleDarkMode"/>

    <!-- 자식 컴포넌트에서 props는 read-only이기 때문에 변경해도 변겨 되지 않으며
    warning 이 발생된다. -->
